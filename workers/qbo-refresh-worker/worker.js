...
// ─── QBO REPORTS API (modernization migration) ────────────────────────────────
// Intuit is moving the Reports API to a modernized backend in two phases
// (Group 1: Jul 1–16, Group 2: Jul 13–22, 2025). We opt in early with
// testing_migration=true so responses don't change shape underneath us on the
// auto-cutover dates. Modernized responses are flagged with a v3modernResponse
// header (and/or top-level field); we log when that appears so the cutover is
// observable. TaxSummary is NOT modernized yet — do not opt it in.
const REPORTS_NOT_MIGRATED = new Set(['TaxSummary']);

async function qboReport(reportName, params, env) {
  let tokens = await getTokens(env);
  if (!tokens.expires_in || tokens.expires_in < 300) tokens = await refreshAccessToken(env);

  const qs = new URLSearchParams(params || {});
  if (!qs.has('minorversion')) qs.set('minorversion', '65');
  if (!REPORTS_NOT_MIGRATED.has(reportName)) qs.set('testing_migration', 'true');

  const url = `${QBO_API_BASE}/${REALM_ID}/reports/${reportName}?${qs.toString()}`;
  const res = await fetch(url, { headers: { 'Authorization': `Bearer ${tokens.access_token}`, 'Accept': 'application/json' } });
  const text = await res.text();
  if (!res.ok) throw new Error(`QBO report ${reportName} error: ${res.status} ${text}`);

  let data;
  try { data = JSON.parse(text); } catch { data = { raw: text }; }

  // Cutover detection: Intuit marks modernized responses with a header and/or
  // a top-level field. Surface it on the response and log so we can alert when
  // the auto-cutover lands.
  const headerFlag = res.headers.get('v3modernResponse') || res.headers.get('intuit-v3modernresponse');
  const modern = headerFlag === 'true' || data?.v3modernResponse === true || data?.Header?.v3modernResponse === true;
  if (modern) console.log(`[qbo-report] ${reportName}: modernized Reports backend ACTIVE (v3modernResponse=true)`);

  // Defensive validation: a real report carries a Header/Columns/Rows shape.
  // Warn (don't throw) if the shape is unexpected so downstream parsers fail loud.
  if (!data || (!data.Header && !data.Columns && !data.Rows && !data.raw)) {
    console.warn(`[qbo-report] ${reportName}: unexpected response shape, keys=${Object.keys(data || {}).join(',')}`);
  }
...
async function fetchProfitLoss(env, h, s, e) {
  const { data, modern } = await qboReport('ProfitAndLoss', { start_date: s, end_date: e }, env);
  return new Response(JSON.stringify(data), { headers: { ...h, 'Content-Type': 'application/json', 'X-QBO-Modern-Response': String(modern) } });
}
